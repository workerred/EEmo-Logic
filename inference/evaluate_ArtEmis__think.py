...
root_dir = "path/to/ArtEmis"
dataset = "ArtEmis"
input_json = "path/to/ArtEmis.json"
output_json = f"path/to/ArtEmis/{dataset}_dominant({model_name}).json"
output_data = load_data(input_json, output_json)
current_index = find_last_generate(output_data, model_name)
emotion_options = list(set([item["dominant_emotion"] for item in output_data]))
...
for index in range(current_index, len(output_data)):
    ...
    item[f"predict({model_name})"] = extract_answer
    if index % 10 == 0:

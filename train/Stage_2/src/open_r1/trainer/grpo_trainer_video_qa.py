...
class Qwen2VLGRPOTrainer_Video_QA(Trainer):
    ...
    def __init__(
        ...
        if isinstance(model, str):
            ...
            elif "Qwen2.5" or "cold_start" in model_id:
                # breakpoint()
        ...
        if processing_class is None:
            if "Qwen2-VL" in model_id or "Qwen2.5" in model_id or "cold_start" in model_id or "Aria" in model_id:
                processing_class = Qwen2_5_VLProcessor.from_pretrained(model_id)
                pad_token_id = processing_class.tokenizer.pad_token_id
                processing_class.pad_token_id = pad_token_id
                processing_class.eos_token_id = processing_class.tokenizer.eos_token_id
                if "Qwen" in model_id or "Qwen2.5" in model_id or "cold_start" in model_id:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # print(prompt_completion_ids.shape,"prompt_ids shape:", prompt_ids.shape, "completion_ids shape:", prompt_completion_ids.shape)
        ...
        # print("log:",per_token_logps)
        # has_nan = torch.isnan(x).any()
        # print(torch.isnan(per_token_logps).any())  # tensor(True)
        with torch.inference_mode():
                if self.ref_model is not None:
                    ...
                    # print(torch.isnan(ref_per_token_logps).any())  # tensor(True)
                else:
        ...
        # except:
        #     pixel_values = [prompt_inputs["pixel_values"][0].repeat(self.num_generations, 1),prompt_inputs["pixel_values"][1].repeat(self.num_generations, 1,1,1)]
        ...
        # print(torch.isnan(ref_per_token_logps).any()) 
        # print("ref:",ref_per_token_logps)
        ...
        #             advantages[num]=0
        # else:
        #     advantages = (rewards - mean_grouped_rewards-0.5*1e-4) / (std_grouped_rewards + 1e-4)
        # else:
        #     advantages = (rewards - mean_grouped_rewards-0.5*1e-4) / (std_grouped_rewards + 1e-4)
        advantages = (rewards - mean_grouped_rewards) / (std_grouped_rewards + 1e-4)
